...
class RiemannPlayerTest {
    private lateinit var riemannPlayer: RiemannPlayer
    ...
    @BeforeEach
    fun setup() {
        riemannPlayer = RiemannPlayer("zekai")
    ...
    @Test
    fun `test riemann player setup with null `() {
        assertThrows<IllegalStateException> {
            riemannPlayer.setup(null, Position(0, 0))
        }
    }

    @Test
    fun `test riemann player take round `() {
        riemannPlayer.setup(state, Position(1, 1))
        assertEquals(
            PlayerSIM(2, SlideDirection.LEFT,
                0, Position(1, 1)
            ), riemannPlayer.takeTurn(state)
        )
    }

    @Test
    fun `test riemann player take round to home`() {
        riemannPlayer.setup(state, Position(1, 1))
        riemannPlayer.setup(null, Position(2, 2))
        assertEquals(
